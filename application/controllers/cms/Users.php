...
class Users extends Admin_core_controller {
  ...
  { 
    ...
    $this->db->where("CONCAT(fname, ' ', lname) LIKE '%{$this->input->get('squery')}%'");
...
#####################
  ...
  function pdf_pedometer_counter($user_id)
  {
    $user = $this->users_model->get($user_id);
    ...
    $pdf->SetCreator(PDF_CREATOR);
    $pdf->SetAuthor('Titowell Admin');
    $pdf->SetTitle("$user->fname $user->lname Pedometer counter"); 
    ...
    // create some HTML content
    $html = '<style>th, td {border: 1px solid black; padding:15px}</style>
    <h3>' .  "$user->fname $user->lname - Pedometer Counter PDF" . '</h3>
    <table class="table table-striped" cellpadding="9">
                <thead>
                  <tr>
                    <th>Date</th>
    ...
    foreach($this->users_model->getPedometerCounter($user) as $value) {
      $html.= "<tr>";
      $html.= "<td>$value->datetime_f</td>";
      $html.= "<td>$value->step_count</td>";
      $html.= "</tr>";
    }           
    $html.='</tbody>
              </table>';
    ...
    //Close and output PDF document
    $pdf->Output($user->lname . '_pedometer_counter.pdf', 'I');
